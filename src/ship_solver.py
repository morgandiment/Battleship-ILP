...
class ShipModelSolver:
    ...
    def solve(self, puzzle):
        ...
        for r in range(size):
            model.addConstr(
                gp.quicksum(y[cid] for cid in row_map[r]) == puzzle.row_tallies[r],
                name=f"Row_Tally_{r}",
            )

        for c in range(size):
            model.addConstr(
                gp.quicksum(y[cid] for cid in col_map[c]) == puzzle.col_tallies[c],
                name=f"Col_Tally_{c}",
            )
        ...
        if hasattr(puzzle, "hints") and puzzle.hints:
            for (r, c_idx), val in puzzle.hints.items():
                ...
                if val == WATER:
                    model.addConstr(
                        gp.quicksum(y[cid] for cid in covering_ids) == 0,
                        name=f"Hint_W_{r}_{c_idx}",
                    )
                    continue
                ...
                # Must be a ship
                model.addConstr(
                    gp.quicksum(y[cid] for cid in covering_ids) == 1,
                    name=f"Hint_S_{r}_{c_idx}",
                )
                ...
                if invalid_cids:
                    model.addConstr(
                        gp.quicksum(y[cid] for cid in invalid_cids) == 0,
                        name=f"Hint_ShapeBan_{r}_{c_idx}",
                    )
