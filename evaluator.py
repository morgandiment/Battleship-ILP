...
def parse_prolog_csplib(filepath):
    ...
    pattern = re.compile(
        r"problem\s*\(\s*(\d+)\s*," r"\s*\[(.*?)\]\s*," r"\s*\[(.*?)\]\s*," r"\s*\[(.*?)\]\s*," r"\s*\[(.*?)\]\s*\)\.",
        re.DOTALL,
    )
...
def plot_cactus_results(ship_times, cell_times, cell_improv_times, timestamp, run_dir):
    ...
    if valid_ship:
        ax.plot(
            ...
            marker="o",
            markersize=4,
            markevery=max(1, len(valid_ship) // 20),
        )
    if valid_cell:
        ax.plot(
            x_cell,
            valid_cell,
            label="Cell Model (Standard)",
            color="red",
            linewidth=2,
            marker="^",
            markersize=4,
            markevery=max(1, len(valid_cell) // 20),
        )
    if valid_cell_improv:
        ax.plot(
            x_cell_improv,
            valid_cell_improv,
            label="Cell Model (Improved)",
            color="seagreen",
            linewidth=2,
            marker="s",
            markersize=4,
            markevery=max(1, len(valid_cell_improv) // 20),
        )
...
def plot_cuts_comparison(sizes, cell_times, cell_improv_times, timestamp, run_dir):
    ...
    ax.bar(
        ...
        avg_cuts,
        width,
        label="Cell Model (With Improvisations)",
        color="seagreen",
        alpha=0.8,
    )
    ...
    ax.set_xlabel("Grid Size (Total Cells)", fontsize=12, fontweight="bold")
    ax.set_title(
        "Impact of Valid Inequalities on Cell Model Performance",
        fontsize=14,
        fontweight="bold",
    )
    ax.set_xticks(x)
...
def run_evaluation(filepath, solver_choice):
    ...
    print(
        f"{'ID':<6} | {'Size':<6} | {'Hints':<5} | {'Diff':<6} | "
        f"{'Ship Time':<10} | {'Cell Time':<10} | {'Cell Improved':<10}"
    )
    print("-" * 70)
    ...
    for puzzle in puzzles:
        ...
        print(
            f"{puzzle.id:<6} | {size}x{size:<4} | {num_hints:<5} | {diff:<6} | "
            f"{ship_status:<10} | {cell_status:<10} | {cell_improv_status:<10}"
        )
    ...
    for diff_level in ["Easy", "Medium", "Hard"]:
        ...
        if solver_choice == "ALL" and count_ships > 0 and count_cells > 0 and count_cell_improv > 0:
            joint_pairs = [
                (s, c, cut)
                for s, c, cut, cat in zip(ship_times, cell_times, cell_improv_times, categories)
                if s > 0 and c > 0 and cut > 0 and cat == diff_level
            ]
            if joint_pairs:
                ...
                print(
                    f"  Win Count -> Ship: {ship_wins:<4} | Cell Std: {cell_wins:<4} | Cell Improved: {cell_improv_wins:<4}"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "filepath",
        type=str,
        help="Path to the CSPLib formatted dataset (e.g., data/test.pl)",
    )
    parser.add_argument(
        "--solver",
        type=str,
        choices=["CELL", "CELL_IMPROVED", "SHIP", "ALL"],
        default="ALL",
        help="Which solver to run: CELL, CELL_IMPROVED, SHIP or ALL (default is ALL)",
    )
